...
Wizard.prototype._setColdWallet = function(config, callback){
  api.setColdWallet(config.ripple_address, function(error, address){
    if(error){
...
Wizard.prototype._setHotWallet = function(callback){
  var self = this;
  api.generateWallet(function(error, wallet){
    if(error){
      callback(error, null);
    } else {
      api.setHotWallet(wallet.address, wallet.secret, function(error, hotWallet){
        if(error){
          callback(error, null)
        } else {
...
Wizard.prototype._fundHotWallet = function (configProperties, callback){
  ...
  api.fundHotWallet(opts, function(error, payment){
    if(error){
      callback(error, null);
    } else {
...
Wizard.prototype._setLastPaymentHash = function(config, callback){
  var self = this;
  api.setLastPaymentHash(config, function(error, response){
    if(error){
      callback(error, null);
    } else {
...
Wizard.prototype._setTrustLine = function(config, callback){
  var self = this;
  for (var currency in config.currencies) {
    api.setTrustLine(currency, config.currencies[currency], function(error, response){
      if(error){
        callback(error, null);
      } else {
...
Wizard.prototype._addCurrency = function(config, callback){
  var self = this;
  for (var currency in config.currencies) {
    api.addCurrency(currency, config.currencies[currency], function (error, response) {
      if (error) {
        callback(error, null);
      } else {
...
Wizard.prototype._issueCurrency = function(config, callback){
  ...
  api.issueCurrency(opts, function (error, response) {
    if (error) {
      callback(error, null);
    } else {
...
Wizard.prototype._setRippleRestUrl = function(config, callback){
  var self = this;
  api.setRippleRestUrl(config, function(error, response){
    if(error){
      callback(error, null);
    } else {
...
Wizard.prototype._setDatabaseUrl = function(config, callback){
  var self = this;
  api.setDatabaseUrl(config, function(error, response){
    if(error){
      callback(error, null);
    } else {
